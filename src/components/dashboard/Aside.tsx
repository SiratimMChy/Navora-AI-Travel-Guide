...
function AsideInner() {
  ...
  return (
    <>
      ...
      {/* Single aside — fixed drawer on mobile, flex item on desktop */}
      <aside
        className={`
          fixed top-0 left-0 h-full z-50
          md:sticky md:top-16 md:h-[calc(100vh-4rem)]
          flex flex-col bg-base-200 border-r-2 border-base-300 shadow-lg
          transition-all duration-300 shrink-0
          ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
          ${isCollapsed ? "w-16" : "w-64"}
        `}
      >
        {/* Collapse toggle — desktop, sits at bottom of brand section */}
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className={`hidden md:flex absolute top-[72px] ${isCollapsed ? "left-1/2 -translate-x-1/2" : "-right-3"} bg-base-100 border-2 border-base-300 rounded-full p-1.5 shadow-md hover:bg-sky-50 hover:border-sky-200 transition-all z-10`}        >
          {isCollapsed
        ...
        <button
          ...
          className="md:hidden absolute top-3 right-3 p-1.5 rounded-full hover:bg-base-200 z-10"
        >
          <X size={18} className="text-base-content/60" />
        </button>

        {/* Brand */}
        <div className="px-4 pt-8 pb-5 border-b-2 border-base-300 bg-base-100">
          <div className={`flex items-center gap-3 ${isCollapsed ? "justify-center" : ""}`}>
            <div className="w-9 h-9 bg-sky-500 rounded-xl flex items-center justify-center shadow-md shrink-0">
              <span className="text-white font-bold text-sm">N</span>
            </div>
            {!isCollapsed && (
              <div className="overflow-hidden">
                <p className="text-sm font-bold text-base-content leading-tight truncate">{title}</p>
                <p className="text-xs text-base-content/40 truncate">{user?.name}</p>
              </div>
            )}
          </div>
        </div>
